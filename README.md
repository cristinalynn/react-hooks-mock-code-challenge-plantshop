...
1. When the app starts, I can see all plants.
  ...
  - Use an array of PlanstCards to display my plant objects
...
2. I can add a new plant to the page by submitting the form.
  ...
  - update state by adding a new plant to the array in our PlantList

3. I can mark a plant as "sold out".
  - add some isInStock state to the plant card
  - toggle that state when the button is clicked
  - conditionallt render a button based on a isInStock state

4. I can search for plants by their name and see a filtered list of plants.
  - add state to keep track of the search term being typed by the user
  - use that state to filter the list of plants in our Plantlist component so only plants that match the search are dysplayed
